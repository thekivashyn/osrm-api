...
export const API_DOC_CATEGORIES: {
...
}[] = [
  {
    ...
    description: "Health, probe OSRM/Pelias, audit IP client.",
  },
  {
    id: "geocode",
    label: "Geocoding",
    description: "Autocomplete & search địa chỉ qua Pelias (Việt Nam, libpostal).",
  },
  {
    id: "routing",
    label: "Routing OSRM",
...
export const API_DOCS: ApiDocEndpoint[] = [
  ...
  },
  {
    ...
    title: "Pelias status",
    description: "Kiểm tra Pelias geocoder sẵn sàng (Elasticsearch + API).",
    category: "system",
    responseWrap: "root",
    response: [
      { name: "success", type: "boolean", description: "true khi Pelias OK" },
      { name: "pelias", type: "ok | down", description: "Trạng thái geocoder" },
      { name: "service", type: "string", description: "Nhãn nội bộ (pelias)" },
      { name: "message", type: "string", description: "Chi tiết (khi lỗi)" },
    ],
    sampleResponse: { success: true, pelias: "ok", service: "pelias" },
  },
  {
    id: "client-ip",
    method: "GET",
  ...
  },
  {
    ...
    description: "Tìm địa chỉ qua Pelias autocomplete/search. Truyền lat/lng để focus gần vùng map.",
    category: "geocode",
    ...
    response: [
      ...
      { name: "data.results[].layer", type: "string | null", description: "Pelias layer" },
    ],
    sampleResponse: {
      success: true,
      data: {
    ...
    errors: [{ status: 502, description: "Pelias unreachable hoặc index chưa import" }],
  },
  {
    id: "reverse-geocode",
    method: "GET",
